...
import type { SummaryData } from "@/types/investment-climate";
...
export function SummaryBlock({
  ...
  return (
    <Section
      id="summary"
      title="Резюме"
      lead="Семь главных выводов аудита и решения, которые стоит принять в первую очередь."
    >
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
          ...
          {summary.findings.map((f, i) => {
            ...
            return (
              <Reveal key={f.id} delay={Math.min(i * 0.06, 0.3)}>
                <article className="ic-card flex h-full flex-col gap-2.5 p-5">
        ...
        <Reveal className="lg:sticky lg:top-[76px]">
          <aside className="ic-card p-5" aria-label="Решить в первую очередь">
            <h3 className="ic-h3 mb-4">Решить в первую очередь</h3>
            <ol className="space-y-3">
              {summary.priorities.map((p) => (
                <li key={p.n} className="flex gap-3 border-t border-[var(--ic-line)] pt-3 first:border-t-0 first:pt-0">
                  ...
                  <div>
                    <div className="text-[13.5px] font-semibold leading-snug">{p.title}</div>
