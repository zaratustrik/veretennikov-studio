...
export default function EnterpriseWhySection({ lite, mobile, ready }: Props) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 5) финальная мысль
      tl.to("#ew-key", { opacity: 1, y: 0, duration: 0.12, ease: ease.ui }, 1.06);
    }, sectionRef);
    return () => ctx.revert();
  }, [ready, lite, mobile]);
          ...
          >
            ...
            {entWhy.materials.map((m, i) => (
              <g key={m.name}>
                ...
                {m.route === "mask" ? (
                  <circle className="ew-mark" data-route="mask" cx="470" cy={211 + i * 44} r="9" fill="none" stroke="var(--accent-delegate)" strokeWidth="1.6" strokeDasharray="3 3" />
                ) : null}
                {m.route === "block" ? (
                  <text className="ew-mark" data-route="block" x="452" y={216 + i * 44} fill="var(--danger)" fontSize="14" fontWeight="700">
                    ×
                  </text>
                ) : null}
                {m.route === "stay" ? (
                  <text className="ew-mark" data-route="stay" x="292" y={216 + i * 44} fill="var(--accent-check)" fontSize="11">
                    остаётся внутри
                  </text>
                ) : null}
              </g>
            ...
            <g id="ew-boundary">
              ...
              <text x="480" y="72" textAnchor="middle" fill="var(--accent-rebuild)" fontSize="12" letterSpacing="2">
            ...
            <g opacity="0.9">
              ...
              <text x="640" y="205" textAnchor="middle" fill="var(--text-secondary)" fontSize="13">
                внешние модели
              </text>
              {[0, 1, 2].map((i) => (
