...
/**
 * Вход: сравнение пароля с env KIT_PROPOSAL_PASSWORD через timingSafeEqual
...
export async function loginAction(
  ...
  if (!secret) {
    return { error: "Доступ не настроен: обратитесь к автору документа." }
  }
  ...
  if (typeof attempt !== "string" || attempt.length === 0) {
    return { error: "Введите пароль доступа." }
  }
  ...
  store.set(KDL_AUTH_COOKIE, expectedCookieValue(secret), {
    httpOnly: true,
    sameSite: "lax",
  ...
  const next =
    typeof rawNext === "string" && rawNext.startsWith(KDL_BASE_PATH)
      ? rawNext
      : KDL_BASE_PATH
  redirect(next)
}
...
export async function logoutAction(): Promise<void> {
  const store = await cookies()
  store.delete({ name: KDL_AUTH_COOKIE, path: KDL_BASE_PATH })
  redirect(KDL_BASE_PATH)
}
